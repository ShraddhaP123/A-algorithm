...
pygame.display.set_caption("Adaptive A*")
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
PURPLE = (128, 0, 128)
ORANGE = (255, 165 ,0)
...
def calculate_g_target(grid, start, end):
	count = 0
...
def astar_adaptive(draw, grid, start, end, g_target):
	count = 0
	open_set = PriorityQueue()
	g_value = {spot: float("inf") for row in grid for spot in row}
	g_value[start] = 0
	f_value = {spot: float("inf") for row in grid for spot in row}
	f_value[start] = h_adaptive(g_value[start], g_target)
	open_set.put((0, g_value[start], start)) #tie breaker situation if f_value is same then node is chosen based on g_value
	came_from = {}
	...
	while not open_set.empty():
		...
		for adjacent in current.adjacent:
			...
			if temp_g_value < g_value[adjacent]:
				...
				g_value[adjacent] = temp_g_value
				f_value[adjacent] = temp_g_value + h_adaptive(g_value[adjacent], g_target)
				if adjacent not in open_set_hash:
					count += 1
					open_set.put((f_value[adjacent], g_value[adjacent], adjacent))
					open_set_hash.add(adjacent)
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_SPACE and start and end:
					...
					g_target = calculate_g_target(grid, start, end)
					t0 = time()
					val = astar_adaptive(lambda: draw(win, grid, ROWS, width), grid, start, end, g_target)
					t1 = time()
