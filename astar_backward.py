...
def construct_shortest_path(came_from, current, draw):
        ...
        while current in came_from:
                ...
                draw()
        if count != 0:
                print ("\nShortest Path from Target to Agent:", count,  "steps\n")
                for row, col in reversed(res):
                        print ("[", row, col, "]", end=' : ', flush=True)

def astar_backwards(draw, grid, start, end):
	count = 0
	...
	open_set_hash = {start}
	print("Open list for traversal in order:\n")
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_SPACE and start and end:
					...
					t0 = time()
					val = astar_backwards(lambda: draw(win, grid, ROWS, width), grid, end, start)
					t1 = time()
					if val==0:
